...
import { Upload, X, Eye, FileText, Plus, Trash2 } from 'lucide-react';
...
const DocumentUploader: React.FC<DocumentUploaderProps> = ({
  ...
  const handleFileSelect = (file: File, type: 'license' | 'idCard' | 'photo' | 'additional', customName?: string) => {
    if (!file) return;
    ...
    if (type === 'license') {
      updatedDocs.drivingLicense = newDoc;
    } else if (type === 'idCard') {
      updatedDocs.idCard = newDoc;
    } else if (type === 'photo') {
      updatedDocs.photo = newDoc;
    } else if (type === 'additional') {
      if (updatedDocs.additional.length < 2) {
        updatedDocs.additional = [...updatedDocs.additional, newDoc];
      } else {
        alert('You can only upload up to 2 additional documents');
        return;
      }
    }
  ...
  const handleRemoveDocument = (type: 'license' | 'idCard' | 'photo' | 'additional', index?: number) => {
    const updatedDocs = { ...documents };

    if (type === 'license') {
      updatedDocs.drivingLicense = null;
    } else if (type === 'idCard') {
      updatedDocs.idCard = null;
    } else if (type === 'photo') {
      updatedDocs.photo = null;
  ...
  const getDocumentTypeName = (type: string) => {
    switch (type) {
      ...
      case 'photo': return 'Driver Photo';
    ...
    <div className="space-y-4">
      <div className="bg-blue-50 p-4 rounded-lg border border-blue-200">
        ...
        <p className="text-sm text-blue-700">
          Please upload clear photos or scans of the required documents. All documents will be securely stored and used only for verification purposes.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <DocumentCard
          doc={documents.drivingLicense}
          type="license"
          onRemove={() => handleRemoveDocument('license')}
          isRequired
        />
        <DocumentCard
          doc={documents.idCard}
          type="idCard"
          onRemove={() => handleRemoveDocument('idCard')}
          isRequired
        />
        <DocumentCard
          doc={documents.photo}
                ...
                <div className="space-y-3">
                  ...
                  <div className="border-2 border-dashed border-gray-300 rounded-lg p-4 text-center hover:border-blue-400 transition-colors">
                    <input
                      ...
                      disabled={isUploading}
                    />
                    <label htmlFor="upload-additional" className="cursor-pointer block">
      ...
      {isUploading && (
        <div className="bg-blue-50 p-4 rounded-lg border border-blue-200">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm text-blue-700">Uploading documents to secure cloud storage...</span>
          </div>
        </div>
      )}
    </div>
