...
import { Clock, CheckCircle, AlertCircle, Calculator, Calendar, TrendingUp, AlertTriangle, RotateCcw, ArrowLeft } from 'lucide-react';
import { LoanDetails, Vehicle, Role } from '@/types/user';
import { useFirebaseData, VehicleFinancialData, Expense } from '@/hooks/useFirebaseData';
...
import { collection, doc, getDoc, getDocs, query, where, orderBy, limit, setDoc, increment } from 'firebase/firestore';

type EMIScheduleItem = LoanDetails['amortizationSchedule'][number];
type SelectedEMI = { monthIndex: number; scheduleItem: EMIScheduleItem };

const createFinancialData = (overrides?: Partial<VehicleFinancialData>): VehicleFinancialData => ({
  outstandingLoan: 0,
  totalEarnings: 0,
  totalExpenses: 0,
  totalInvestment: 0,
  ...
  totalReturn: 0,
  roiPercentage: 0,
  roiVsInvestmentPercentage: 0,
  isROIGreaterThanInvestment: false,
  totalEmiPaid: 0,
  totalInvestmentWithPrepayments: 0,
  isInvestmentCovered: false,
  grossProfitLoss: 0,
  ...
  partnerShareMonthlyProfit: 0,
  partnerShareMonthlyLoss: 0,
  ownerShareMonthlyProfit: 0,
  ownerShareMonthlyLoss: 0,
  ...overrides,
});
...
const StandaloneEMITabPage: React.FC = () => {
  const [searchParams] = useSearchParams();
  const navigate = useNavigate();
  const vehicleId = searchParams.get('vehicleId');
  const { vehicles, loading, updateVehicle, addExpense } = useFirebaseData();
  const { userInfo } = useAuth();
  const paths = useFirestorePaths(userInfo?.companyId);
  const { toast } = useToast();
  
  // Penalty dialog state (same as VehicleDetails)
  ...
  const financialData: VehicleFinancialData = useMemo(() => 
  ...
  // EXACT COPY of processEMIPayment from VehicleDetails.tsx (lines 931-1064)
  ...
  // EXACT COPY of reverseEMIPayment from VehicleDetails.tsx (lines 1066-1330)
  const reverseEMIPayment = async (monthIndex: number, scheduleItem: EMIScheduleItem) => {
    ...
    try {
      ...
      let originalEmiAmount = emiAmount;
      try {
      ...
      // Mark EMI as unpaid
      const { paidAt: _paidAt, ...emiWithoutPaidAt } = updatedSchedule[monthIndex];
      updatedSchedule[monthIndex] = {
        ...emiWithoutPaidAt,
        isPaid: false
      };
      ...
      // Create reversing expense entries
  ...
  if (loading) {
    return <div className="p-6">Loading vehicle data...</div>;
  }

  if (!vehicle) {
    return (
      <div className="min-h-screen bg-gray-50 p-6 flex items-center justify-center">
        <div className="max-w-lg w-full bg-white rounded-lg shadow p-6 text-center space-y-4">
          <h2 className="text-xl font-semibold">Vehicle not found</h2>
          <p className="text-gray-600">Select a vehicle from the utility page to view EMI details.</p>
          <Button onClick={() => navigate('/utility')}>
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="bg-white border-b p-4 space-y-4">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-xl font-semibold">EMI Management - {vehicle.vehicleName || vehicle.registrationNumber || 'Vehicle'}</h1>
        </div>
        ...
        <div className="overflow-x-auto pb-2 px-1 pt-1">
          <div className="flex gap-3 min-w-max">
            {vehicles.map((v) => (
              <div
                key={v.id}
                onClick={() => navigate(`/emi?vehicleId=${v.id}`)}
      ...
      {/* Penalty Payment Dialog (EXACT COPY from VehicleDetails lines 2400-2443) */}
      {userInfo?.role !== Role.PARTNER && (
...
export default StandaloneEMITabPage;