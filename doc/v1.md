...
### 4.1 Core Data Management Hook (`useFirebaseData.ts`)

The `useFirebaseData.ts` file contains multiple specialized hooks for managing different aspects of the car rental system data, plus a main hook that combines them all for easy access.

#### Individual Entity Hooks

##### `useDrivers()` Hook
**Purpose**: Manages driver data with real-time synchronization
```typescript
export const useDrivers = () => {
  const [drivers, setDrivers] = useState<Driver[]>([]);
  const [loading, setLoading] = useState(true);
  // Real-time listener for drivers collection
  // Returns: { drivers, loading, addDriver, updateDriver, deleteDriver }
};
```

**Key Features**:
- Real-time driver data fetching from `users` collection where `userType === 'Driver'`
- CRUD operations: add, update, delete drivers
- Automatic company isolation using `companyId`

##### `useVehicles()` Hook
**Purpose**: Manages vehicle fleet data with financial calculations
```typescript
export const useVehicles = () => {
  const [vehicles, setVehicles] = useState<Vehicle[]>([]);
  const [loading, setLoading] = useState(true);
  // Real-time listener for vehicles collection ordered by registrationNumber
  // Returns: { vehicles, loading, addVehicle, updateVehicle, deleteVehicle }
};
```

**Key Features**:
- Real-time vehicle data fetching with ordering
- CRUD operations for vehicle management
- Company-scoped data access

##### `useAssignments()` Hook
**Purpose**: Manages rental assignments between vehicles and drivers
```typescript
export const useAssignments = () => {
  const [assignments, setAssignments] = useState<Assignment[]>([]);
  const [loading, setLoading] = useState(true);
  // Real-time listener ordered by startDate descending
  // Returns: { assignments, loading, addAssignment, updateAssignment }
};
```

**Key Features**:
- Auto-calculates weekly rent from daily rent
- Real-time assignment tracking
- Status-based filtering capabilities

##### `usePayments()` Hook
**Purpose**: Manages rent payment collection and tracking
```typescript
export const usePayments = () => {
  const [payments, setPayments] = useState<Payment[]>([]);
  const [loading, setLoading] = useState(true);
  // Real-time listener ordered by nextDueDate descending
  // Returns: { payments, loading, markPaymentCollected }
};
```

**Key Features**:
- Calculates days left until payment is due
- Payment status management ('due', 'paid', 'overdue')
- Collection date tracking

##### `useExpenses()` Hook
**Purpose**: Manages all vehicle-related expenses with cash balance updates
```typescript
export const useExpenses = () => {
  const [expenses, setExpenses] = useState<Expense[]>([]);
  const [loading, setLoading] = useState(true);
  // Real-time listener ordered by createdAt descending
  // Returns: { expenses, loading, addExpense, updateExpense, deleteExpense }
};
```

**Key Features**:
- Hierarchical expense categorization (`paymentType`, `expenseType`)
- Automatic cash balance updates when expenses are approved
- Prorated calculations for advance payments
- Status workflow: pending → approved/rejected

##### `usePartners()` Hook
**Purpose**: Manages partner users in the system
```typescript
export const usePartners = () => {
  const [partners, setPartners] = useState<UserInfo[]>([]);
  const [loading, setLoading] = useState(true);
  // Real-time listener for users with role === 'PARTNER'
  // Returns: { partners, loading }
};
```

**Key Features**:
- Partner-specific user filtering
- Real-time partner data synchronization
- Company-scoped partner management

##### `useFuelRecords()` Hook
**Purpose**: Manages fuel consumption records for vehicles
```typescript
export const useFuelRecords = () => {
  const [fuelRecords, setFuelRecords] = useState<FuelRecord[]>([]);
  const [loading, setLoading] = useState(true);
  // Real-time listener ordered by date descending
  // Returns: { fuelRecords, loading, addFuelRecord }
};
```

##### `useMaintenanceRecords()` Hook
**Purpose**: Manages vehicle maintenance and service records
```typescript
export const useMaintenanceRecords = () => {
  const [maintenanceRecords, setMaintenanceRecords] = useState<MaintenanceRecord[]>([]);
  const [loading, setLoading] = useState(true);
  // Real-time listener ordered by date descending
  // Returns: { maintenanceRecords, loading, addMaintenanceRecord }
};
```

##### `useTenantCompanies()` Hook
**Purpose**: Manages tenant companies for multi-tenant architecture
```typescript
export const useTenantCompanies = () => {
  const [companies, setCompanies] = useState<any[]>([]);
  const [loading, setLoading] = useState(true);
  // Real-time listener for tenant companies
  // Returns: { companies, loading }
};
```

##### `useDashboardStats()` Hook
**Purpose**: Calculates dashboard statistics from all data sources
```typescript
export const useDashboardStats = () => {
  // Combines data from useDrivers, useVehicles, useAssignments, usePayments
  const stats = {
    totalVehicles, rentedVehicles, availableVehicles, maintenanceVehicles,
    totalDrivers, activeDrivers,
    activeAssignments,
    pendingPayments, overdue,
    totalWeeklyRent, totalOutstandingRent,
    utilizationRate
  };
...
#### Main `useFirebaseData()` Hook
**Purpose**: Combines all individual hooks and provides enhanced financial calculations
```typescript
export const useFirebaseData = () => {
  // Combines all individual hooks
  const {
    drivers, vehicles, assignments, payments, expenses, partners,
    loading,
    addDriver, addVehicle, updateVehicle, addAssignment,
    addExpense, updateExpense, deleteExpense, markPaymentCollected, deleteVehicle
  } = // ...individual hooks
  ...
  const getVehicleFinancialData = (vehicleId: string): VehicleFinancialData | null => {
    // Complex financial calculations including ROI, partnership shares, etc.
  };
  ...
  return {
    // Data
    drivers, vehicles, vehiclesWithFinancials, assignments, payments, expenses, partners, loading,
    
    // Helper Functions
    getVehicleFinancialData,
    
    // Actions
    addDriver, addVehicle, updateVehicle, addAssignment, addExpense, updateExpense, deleteExpense, markPaymentCollected, deleteVehicle
  };
};
```

### 4.2 Firestore Path Management (`useFirestorePaths.ts`)

**Purpose**: Centralized path management for multi-tenant Firestore architecture

```typescript
export const useFirestorePaths = (companyId?: string) => {
  return useMemo(() => ({
    // Base paths
    basePath: 'Easy2Solutions/companyDirectory',
    superAdminPath: 'Easy2Solutions/companyDirectory/superAdmins',
    commonUsersPath: 'Easy2Solutions/companyDirectory/users',
    tenantCompaniesPath: 'Easy2Solutions/companyDirectory/tenantCompanies',
    
    // Dynamic paths for current company
    getUsersPath: () => companyId ? `Easy2Solutions/companyDirectory/tenantCompanies/${companyId}/users` : '',
    getVehiclesPath: () => companyId ? `Easy2Solutions/companyDirectory/tenantCompanies/${companyId}/vehicles` : '',
    getAssignmentsPath: () => companyId ? `Easy2Solutions/companyDirectory/tenantCompanies/${companyId}/assignments` : '',
    getExpensesPath: () => companyId ? `Easy2Solutions/companyDirectory/tenantCompanies/${companyId}/expenses` : '',
    getPaymentsPath: () => companyId ? `Easy2Solutions/companyDirectory/tenantCompanies/${companyId}/payments` : '',
    getFuelPricesPath: () => companyId ? `Easy2Solutions/companyDirectory/tenantCompanies/${companyId}/fuelPrices` : '',
    getFuelRecordsPath: () => companyId ? `Easy2Solutions/companyDirectory/tenantCompanies/${companyId}/fuelRecords` : '',
    getMaintenanceRecordsPath: () => companyId ? `Easy2Solutions/companyDirectory/tenantCompanies/${companyId}/maintenanceRecords` : '',
    getInsuranceRecordsPath: () => companyId ? `Easy2Solutions/companyDirectory/tenantCompanies/${companyId}/insuranceRecords` : '',
    
    // Vehicle subcollections
    getOdometerHistoryPath: (vehicleId: string) => 
      companyId ? `Easy2Solutions/companyDirectory/tenantCompanies/${companyId}/vehicles/${vehicleId}/odometerHistory` : '',
  }), [companyId]);
};
```
...
- Company-scoped path construction
- Support for subcollections (odometer history)
- Consistent path structure across the application

### 4.3 Fuel Price Management (`useFuelPrices.ts`)

**Purpose**: Manages fuel prices and cost calculations for different fuel types
...
export const useFuelPrices = () => {
  ...
  // Real-time fuel prices listener
  // Returns: { fuelPrices, loading, updateFuelPrice, getFuelPrice, calculateFuelCost }
};
```

**Key Features**:
- Real-time fuel price synchronization
- Support for multiple fuel types (Diesel, Petrol, CNG, Electric)
- Fuel cost calculation based on vehicle mileage and distance
- Company-scoped fuel price management
...
const calculateFuelCost = (vehicleMileage: number, mileageUnit: 'kmpl' | 'kmkg', distance: number, fuelType: string): number => {
  ...
  let pricePerUnit = mileageUnit === 'kmpl' ? fuelPrice.pricePerLiter : (fuelPrice.pricePerKg || fuelPrice.pricePerLiter);

  return fuelConsumption * pricePerUnit;
};
```

### 4.4 UI Utility Hooks

#### `useIsMobile()` Hook (`use-mobile.tsx`)
**Purpose**: Detects mobile devices for responsive design
```typescript
export function useIsMobile() {
  const [isMobile, setIsMobile] = useState<boolean | undefined>(undefined);
  // Media query listener for screen width < 768px
  return !!isMobile;
};
```
...
export function useToast() {
  ...
  return {
    toasts: ToasterToast[],
    toast: (props: Toast) => ToastInstance,
...
**Key Features**:
- Toast queue management with limit
- Auto-dismiss functionality
- Update and dismiss actions
- Memory-based state management for cross-component communication

### 4.5 Authentication Context (`AuthContext`)
